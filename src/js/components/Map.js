  ...
  provincePaths.forEach((path) => {
    path.addEventListener("click", () => {
      ...
      const provinceDisplayName =
        provinceTitleMap[provinceNameEn]?.[lang] ||
        `${provinceNameMap[provinceNameEn]?.[lang]} ${t("modal.province")}`;
      ...
      if (matchedFoods.length > 0) {
        const foods = matchedFoods.map((item) => ({
          ...
          food_name:
            typeof item.food_name === "object"
              ? item.food_name?.[lang] || item.food_name?.en
              : item.food_name,
        }));
