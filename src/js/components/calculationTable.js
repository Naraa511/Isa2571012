...
export function renderCalculationTable(
  selectedFoods = [],
  ageOptions = [],
  selectedAge = "",
  selectedGender = "",
...
function updateRowValues(code) {
  ...
  document
    .getElementById(`protein-${code}`)
    ?.replaceChildren(protein.toFixed(2));
  document.getElementById(`fat-${code}`)?.replaceChildren(fat.toFixed(2));
  document
    .getElementById(`carbohydrate-${code}`)
    ?.replaceChildren(carbohydrate.toFixed(2));

  document
    .getElementById(`micro-amount-${code}`)
    ?.replaceChildren(amount.toFixed(2));
  document
    .getElementById(`calcium-${code}`)
    ?.replaceChildren(calcium.toFixed(2));
  document.getElementById(`iron-${code}`)?.replaceChildren(iron.toFixed(2));
  document
    .getElementById(`phosphorus-${code}`)
    ?.replaceChildren(phosphorus.toFixed(2));
  document
    .getElementById(`vitaminc-${code}`)
    ?.replaceChildren(vitaminC.toFixed(2));
}
...
export async function updateRequirementSummary(container = document) {
  ...
  const reqCarbohydrate = Number(
    requirement?.proximates?.["Carbohydrate (g)"] ?? 0,
  );
  ...
  setText(
    "#selected-age-micro",
    typeof ageObject === "object" ? getLocalizedValue(ageObject) : ageObject,
  );
  ...
  setText(
    "#percent-carbohydrate",
    formatPercent(totals.carbohydrate, reqCarbohydrate),
  );
  ...
  setText(
    "#percent-phosphorus",
    formatPercent(totals.phosphorus, reqPhosphorus),
  );
...
function bindRequirementEvents(container) {
  container
    .querySelector("#requirement-age")
    ?.addEventListener("change", async () => {
      await updateRequirementSummary(container);
    });

  container
    .querySelector("#requirement-gender")
    ?.addEventListener("change", async () => {
      await updateRequirementSummary(container);
    });
}
...
export async function refreshCalculationTable(nutritionData) {
  ...
  const currentGender =
    resultBox.querySelector("#requirement-gender")?.value || "";
